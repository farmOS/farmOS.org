...
const findParentNode = (node, parentURI = '/', indices = []) => {
  ...
  if (pathname) {
    ...
    const pathLength = pathToArray(parentURI).length;
    const ancestor = pathToArray(pathname).slice(0, pathLength).join('/');
    const hasCommonAncestor = fmtPath(parentURI) === fmtPath(ancestor);
    if (hasCommonAncestor) return indices;
  }
  for (let i = 0; i < node.children.length; i++) {
    const sibling = node.children[i];
    const siblingIndices = findParentNode(sibling, parentURI, [...indices, i]);
...
const fromSource = source => {
  const { title, config, parentURI, baseURI, children } = source;
  const sourceURI = fmtPath(`${parentURI || ''}/${baseURI}`);
  let nav = {
    title,
    key: sourceURI,
    page: null,
  ...
  if (configYaml && Array.isArray(configYaml.nav)) {
    nav = fromConfigYaml(configYaml, sourceURI);
  }
  if (!Array.isArray(children) || children.length < 1) return nav;
  return children.reduce((parentNav, childConfig) => {
    const indices = findParentNode(parentNav, childConfig.parentURI);
